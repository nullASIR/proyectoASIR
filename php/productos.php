    ...
    <div class="container main-content">
        ...
        <div class="search-filter" style="margin-bottom: 40px; display: flex; justify-content: center; width: 100%;">
            <form method="GET" action="productos.php"
                style="display: flex; gap: 8px; width: 100%; max-width: 450px; background: rgba(30, 41, 59, 0.7); padding: 6px; border-radius: 50px; border: 1px solid rgba(148, 163, 184, 0.2); box-shadow: 0 4px 20px rgba(0,0,0,0.15); align-items: center; box-sizing: border-box;">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="2"
                    stroke-linecap="round" stroke-linejoin="round" style="margin-left: 15px; flex-shrink: 0;">
                    <circle cx="11" cy="11" r="8"></circle>
                    <line x1="21" y1="21" x2="16.65" y2="16.65"></line>
                </svg>
                <input type="text" name="q" placeholder="Buscar productos..."
                    value="<?php echo isset($_GET['q']) ? htmlspecialchars($_GET['q']) : ''; ?>"
                    style="flex: 1; padding: 10px 10px; border-radius: 50px; border: none; background: transparent; color: #fff; outline: none; font-size: 15px; min-width: 0;">
                <button type="submit" class="btn btn-primary"
                    style="border-radius: 50px; padding: 10px 25px; margin: 0; flex-shrink: 0; white-space: nowrap;">Buscar</button>
            </form>
        </div>
                            ...
                            <?php if (isset($_SESSION['usuario_id'])): ?>
                                <div style="display:flex; gap:10px;">
                                    <button id="wishlist-btn-<?php echo $row['IdProducto']; ?>"
                                        style="background: #e74c3c; padding: 8px 12px; border:none; border-radius:4px; cursor:pointer;"
